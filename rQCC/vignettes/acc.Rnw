...
various traditional attribute control charts such as $p$, $c$ and $u$ can be easily
...
The control limits of the $c$ chart is also easily obtained as follows.
...
For more details, see Kaminsky et al.\ \cite{Kaminsky/etc:1992} and Park and Wang~\cite{Park/Wang:2022}.
\begin{alignat}{2}
...
Thus, the $\mathrm{LCL}$ can not be smaller than $n_k a$.
...
minimum variance unbiased (MVU) estimators~\cite{Park/Wang:2022} which are given by 
\[
...
\]
as the MVU estimator, but Park and Wang~\cite{Park/Wang:2022} showed that 
...
Park and Wang~\cite{Park/Wang:2022} showed that the MVU estimators of $\mu$ and $\sigma^2$ are given by 
\[
...
The control limits of the $g$ or $h$ chart is also easily obtained as follows.
